...
export default async function handler( req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (checkUser) {
            ...
            let getStore: UsersSubJectInventory[] = []
            await checkGrades.results.map((item: any) => {
                getStore.push(...item.grade)
            })

            let catchAdd = {
                section: "ไม่จัดหมวดหมู่",
                group_: "ไม่จัดหมวดหมู่"
            }

            // Add catchAdd in getStore evert element
            getStore = getStore.map((item: any) => {
                return { ...item, ...catchAdd }
            })

            await UsersStatus.updateOne({ user_id: jwt_user_id }, {
                user_inventory: getStore,
            })
